...
type PaymentOrder struct {
	...
	// UpgradeSourceSubscriptionID holds the value of the "upgrade_source_subscription_id" field.
	UpgradeSourceSubscriptionID *int64 `json:"upgrade_source_subscription_id,omitempty"`
	// UpgradeSnapshot holds the value of the "upgrade_snapshot" field.
	UpgradeSnapshot map[string]interface{} `json:"upgrade_snapshot,omitempty"`
...
func (*PaymentOrder) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case paymentorder.FieldUpgradeSnapshot, paymentorder.FieldProviderSnapshot:
			values[i] = new([]byte)
		...
		case paymentorder.FieldID, paymentorder.FieldUserID, paymentorder.FieldPlanID, paymentorder.FieldSubscriptionGroupID, paymentorder.FieldSubscriptionDays, paymentorder.FieldUpgradeSourceSubscriptionID:
...
func (_m *PaymentOrder) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case paymentorder.FieldUpgradeSourceSubscriptionID:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field upgrade_source_subscription_id", values[i])
			} else if value.Valid {
				_m.UpgradeSourceSubscriptionID = new(int64)
				*_m.UpgradeSourceSubscriptionID = value.Int64
			}
		case paymentorder.FieldUpgradeSnapshot:
			if value, ok := values[i].(*[]byte); !ok {
				return fmt.Errorf("unexpected type %T for field upgrade_snapshot", values[i])
			} else if value != nil && len(*value) > 0 {
				if err := json.Unmarshal(*value, &_m.UpgradeSnapshot); err != nil {
					return fmt.Errorf("unmarshal field upgrade_snapshot: %w", err)
				}
			}
...
func (_m *PaymentOrder) String() string {
	...
	builder.WriteString(", ")
	if v := _m.UpgradeSourceSubscriptionID; v != nil {
		builder.WriteString("upgrade_source_subscription_id=")
		builder.WriteString(fmt.Sprintf("%v", *v))
	}
	builder.WriteString(", ")
	builder.WriteString("upgrade_snapshot=")
	builder.WriteString(fmt.Sprintf("%v", _m.UpgradeSnapshot))
	builder.WriteString(", ")
